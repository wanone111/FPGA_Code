...
__attribute__((sdx_kernel("fir_top", 0))) void fir_top(data_t *y, data_t *x, coef_t *coef, int len);
# 2 "../src/fir.cpp" 2
...
#pragma HLS PIPELINE II=1
 VITIS_LOOP_15_1: for (i = 99 - 1; i >= 0; i--)
    {
        if (i == 0)
        {
            acc += x * c[0];
...
__attribute__((sdx_kernel("fir_top", 0))) void fir_top(data_t *y, data_t *x, coef_t *coef, int len)
{
#line 38 "/home/aa/FPGA_space/FIR/build_linux/run_hls.tcl"
...
#pragma HLS INTERFACE s_axilite port=return
 ...
 VITIS_LOOP_40_1: for (int i =0; i < 99; i++)
 {
#pragma HLS PIPELINE II=1
 c[i] = *coef++;
 }

 VITIS_LOOP_46_2: for (int i = 0; i < len; i++)
 {
#pragma HLS PIPELINE II=1
 fir(&res,*x++);
  *y = res;
